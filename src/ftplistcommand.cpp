...
FtpListCommand::FtpListCommand(QObject *parent, const QString &fileName, bool nameListOnly) :
    FtpCommand(parent)
{
    qDebug() << "FtpListCommand::FtpListCommand:  " << fileName;
...
{
    qDebug() << "FtpListCommand::startImplementation:  ";
...
void FtpListCommand::listNextBatch()
{
    ...
    while (index < stop)
    {
        ...
        if (socket->write(line.toUtf8()) < 0)
            qDebug() << "FtpListCommand::listNextBatch:  socket->write error:  " << socket->errorString();
